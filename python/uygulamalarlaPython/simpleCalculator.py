import tkinter as tk

def on_button_click(value):
    current = str(entry.get())
    entry.delete(0, tk.END)
    entry.insert(tk.END, current + str(value))

def clear_entry():
    entry.delete(0, tk.END)

def calculate():
    try:
        result = eval(entry.get())
        entry.delete(0, tk.END)
        entry.insert(tk.END, str(result))
    except Exception as e:
        entry.delete(0, tk.END)
        entry.insert(tk.END, "Hata")

def backspace():
    current = entry.get()[:-1]
    entry.delete(0, tk.END)
    entry.insert(tk.END, current)

def change_sign():
    current = entry.get()
    if current and current[0] == '-':
        entry.delete(0)
    else:
        entry.insert(0, '-')

# Ana pencereyi oluşturma
root = tk.Tk()
root.title("Gelişmiş Hesap Makinesi")

# Giriş kutusu oluşturma
entry = tk.Entry(root, width=16, font=('Arial', 14))
entry.grid(row=0, column=0, columnspan=4)

# Butonları oluşturma
buttons = [
    '7', '8', '9', '/',
    '4', '5', '6', '*',
    '1', '2', '3', '-',
    '0', '.', '=', '+'
]

row_val = 1
col_val = 0

for button in buttons:
    tk.Button(root, text=button, padx=20, pady=20, font=('Arial', 12),
              command=lambda b=button: on_button_click(b) if b != '=' else calculate()).grid(row=row_val, column=col_val)
    col_val += 1
    if col_val > 3:
        col_val = 0
        row_val += 1

# Ek fonksiyon butonları
tk.Button(root, text='C', padx=20, pady=20, font=('Arial', 12), command=clear_entry).grid(row=row_val, column=col_val, columnspan=2)
col_val += 2
tk.Button(root, text='⌫', padx=20, pady=20, font=('Arial', 12), command=backspace).grid(row=row_val, column=col_val)
col_val += 1
tk.Button(root, text='+/-', padx=20, pady=20, font=('Arial', 12), command=change_sign).grid(row=row_val, column=col_val)

# Pencereyi ekranda gösterme
root.mainloop()